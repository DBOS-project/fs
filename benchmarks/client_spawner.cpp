...
// compile with g++ -o <output> -std=c++11 -pthread multi_user_multi_cores.cpp
...
int main(int argc, char** argv) {
	...
	start_client(bench, suffix, args, 42);

    vector<thread> threads;

    for (int i = 0; i < user_cnt; i++) {
        threads.push_back(thread(start_client, bench, suffix, string args, i));
    }
