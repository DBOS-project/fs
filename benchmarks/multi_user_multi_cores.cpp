...
void start_client(int user, int transactions, int all_users) { 
    string cmd = "single_user/synchronous/read_bench/run_client.sh -u user" + to_string(user) + " -t " + to_string(transactions) + " > outputs/" + to_string(all_users) + "/user" + to_string(user) + "r.out";
    system(cmd.c_str());
} 
